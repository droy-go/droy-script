...
static void run_repl(void) {
    ...
    if (!state) {
        fprintf(stderr, "Error: Failed to create interpreter state\n");
        return;
    }
    ...
    while (1) {
        ...
        /* FIXED: Check len > 0 before accessing input[len-1] */
        size_t len = strlen(input);
        if (len > 0 && input[len-1] == '\n') {
            input[len-1] = '\0';
            len--;  /* Update len after modification */
        }
        ...
        /* FIXED: Check buffer bounds before strcat */
